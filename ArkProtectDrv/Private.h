...
BOOLEAN APGetNtosExportVariableAddress(IN const WCHAR *wzVariableName, OUT PVOID *VariableAddress);

BOOLEAN APIsUnicodeStringValid(IN PUNICODE_STRING uniString);

VOID APCharToWchar(IN CHAR * szString, OUT WCHAR * wzString);

VOID APPageProtectOff();

VOID APPageProtectOn();

BOOLEAN APDosPathToNtPath(IN WCHAR * wzDosFullPath, OUT WCHAR * wzNtFullPath);

UINT32 APQueryDosDevice(WCHAR *DeviceName, WCHAR *TargetPath, UINT32 MaximumLength);
